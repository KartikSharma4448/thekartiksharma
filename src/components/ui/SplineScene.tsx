...
export const SplineScene: FC<SplineSceneProps> = ({ scene, className }) => {
    ...
    useEffect(() => {
        if (isLowPowerMode || isMobile || !containerRef.current) return;
    ...
    }, [isLowPowerMode, isMobile, isSceneLoaded]);
        ...
        return (
            <div className={`relative w-full h-full bg-background overflow-hidden ${className || ''}`}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-48 h-48 sm:w-64 sm:h-64 bg-primary/10 blur-[90px] rounded-full animate-pulse" />
                </div>
            </div>
        );
    }
