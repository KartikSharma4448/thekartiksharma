...
import { Github, Linkedin, Instagram, ArrowDown, ArrowDownRight, Terminal, Zap, ExternalLink, MessageSquare } from 'lucide-react';
...
export function HeroVisual({ isExiting = false }: { isExiting?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <main className="relative flex-1 flex flex-col justify-center pt-24 sm:pt-32 md:pt-40 pb-12 sm:pb-20 z-10 max-w-[105rem] w-full mx-auto">
        <div className="flex relative gap-2 sm:gap-4 px-3 sm:px-6 md:items-center w-full flex-col justify-center">
          ...
          <AnimatePresence>
            {tooltip.show && (
              ...
              >
                ...
                {tooltip.icon === 'terminal' && <Terminal className="w-4 h-4 text-emerald-400" />}
          ...
          {/* Line 1: KARTIK */}
          <div className="flex flex-col md:flex-row gap-2 sm:gap-4 md:gap-8 md:items-center relative">
            <motion.p
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.8, delay: 0.2 }}
              className="text-[9px] sm:text-[10px] md:text-xs text-muted-foreground text-start md:text-right leading-relaxed max-w-[260px] md:max-w-[220px] font-medium uppercase tracking-[0.18em] px-2 sm:px-4 md:px-0"
            >
            ...
            <div className="relative">
              ...
              <motion.h1
                ...
                className="text-[clamp(2.4rem,8.4vw,12rem)] font-black leading-[0.88] tracking-tighter text-shiny will-change-transform px-2 sm:px-4"
              >
                KARTIK
              </motion.h1>
            </div>
          ...
          <div className="flex gap-4 md:gap-8 items-center relative">
            <div className="relative">
              ...
              <motion.h1
                ...
                className="text-[clamp(2.4rem,8.4vw,12rem)] flex items-center font-black leading-[0.88] tracking-tighter text-shiny will-change-transform px-2 sm:px-4 whitespace-nowrap"
              >
                <span>FULL</span>
                <div
                  ref={zapRef}
                  className="hidden lg:block mx-[0.05em] relative cursor-pointer group shrink-0"
                  onClick={() => window.open(personal.socialLinks.find(s => s.platform === 'GitHub')?.url || 'https://github.com/KartikSharma4448', '_blank')}
                ...
                <div
                  ref={zapSmallRef}
                  className="block lg:hidden mx-[0.04em] relative cursor-pointer group shrink-0"
                  onClick={() => window.open(personal.socialLinks.find(s => s.platform === 'GitHub')?.url || 'https://github.com/KartikSharma4448', '_blank')}
                  onMouseEnter={(e) => setTooltip({ show: true, text: "Explore Projects", icon: 'zap', x: e.clientX, y: e.clientY })}
                  onMouseMove={(e) => setTooltip(prev => ({ ...prev, x: e.clientX, y: e.clientY }))}
                  onMouseLeave={() => setTooltip(prev => ({ ...prev, show: false }))}
          ...
          <div className="flex flex-col md:flex-row gap-2 sm:gap-4 md:gap-8 md:items-center relative">
            ...
            >
              ...
              <div
                ref={botRef}
                className="mx-[0.05em] relative cursor-pointer group shrink-0"
                onClick={() => {
                  window.dispatchEvent(new CustomEvent('portfolio:toggle-terminal'));
                }}
                onMouseEnter={(e) => setTooltip({ show: true, text: "Launch Developer CLI Terminal", icon: 'terminal', x: e.clientX, y: e.clientY })}
                onMouseMove={(e) => setTooltip(prev => ({ ...prev, x: e.clientX, y: e.clientY }))}
                onMouseLeave={() => setTooltip(prev => ({ ...prev, show: false }))}
              >
                <Terminal className="w-[0.85em] h-[0.85em] text-emerald-400 group-hover:text-emerald-300 group-hover:drop-shadow-[0_0_12px_rgba(52,211,153,0.6)] transition-all" strokeWidth={2} />
              </div>
              <span>LOPER</span>
            </motion.h1>
            ...
            <motion.p
              ...
              className="text-[9px] sm:text-[10px] md:text-xs text-muted-foreground pt-1 md:pt-8 leading-relaxed max-w-[280px] md:max-w-[200px] font-medium uppercase tracking-widest px-2 sm:px-4 md:px-0"
            >
          ...
          {showProfile && (
            ...
            >
              <ProfileCard
                ...
                description={`${personal.name} is a dedicated Full Stack, MERN Stack & Mobile App Developer focused on building scalable web architectures, modern Flutter apps, and robust backend systems.`}
