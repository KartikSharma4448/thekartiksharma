...
function buildSystemPrompt(locale: string = 'en'): string {
    const { personal, projects, experiences, experience, education, skills, achievements, softSkills, tools } = portfolioData as any;
    ...
    const expList = (experiences ?? experience ?? [])
        .map((e: any) =>
            `- ${e.role ?? e.position} at ${e.company} (${e.duration ?? e.startDate}): ${e.description ?? (e.responsibilities ?? []).join('; ')}`
        )
        .join('\n');

    const eduList = (education ?? [])
        .map((e: any) =>
            `- ${e.degree} in ${e.major} at ${e.institution} (${e.startDate} - ${e.endDate || 'Present'}, ${e.gpa || ''}).`
        )
        .join('\n');
